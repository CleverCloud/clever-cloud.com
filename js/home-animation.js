  ...
  function launch() {
    ...
    img.onload = function(e){
      for (var i = -size; i < size; i++) {
        ...
        createjs.Tween.get(store["wave" + i], {loop:true}).to({x:canvas.width - (300 * (Math.floor(size) + i)), y:0}, 242704);
        createjs.Tween.get(store["wave2"+i], {loop:true}).to({x:canvas.width - (280 * (Math.floor(size2) + i)), y:0}, 199000);
        createjs.Tween.get(store["wave3"+i], {loop:true}).to({x:canvas.width - (500 * (Math.floor(size3) + i)), y:0}, 150705);
